...
export default class LineGame extends cc.Component {
    ...
    onLoad() {
        ...
        UserInfo.stepUsed = 0;
        this.timeLimit = 10 * UserInfo.currentLevel;

        this._timerCallback = () => {
            if (this._isGameOver) {
                this.unschedule(this._timerCallback); // Stop timer if game over
                return;
            }
            ...
            let steps: String = "";
            if(this.lang == "en"){
                UserInfo.stepUsed == 0? steps = " step": steps = " steps";
            } else {
               steps = "Bước";
            }
            this.stepLabel.string = UserInfo.stepUsed + steps.toString();
            if (UserInfo.timePassed > this.timeLimit) {
                this._isGameOver = true;
                this.showTimeoutPopup();
                this.unschedule(this._timerCallback); // Stop timer when timeout
            }
        };
    ...
    initLangButton() {
       ...
       let level = this.node.getChildByName("Level").getComponent(cc.Label);
       let step = this.node.getChildByName("Step").getComponent(cc.Label);
       let time = this.node.getChildByName("Time").getComponent(cc.Label);
       let helpText = this.node.getChildByName("help").getChildByName("New Label").getComponent(cc.Label);
    
       level.string = t("level");
       step.string = t("step");
       time.string = t("time");
       helpText.string = t("help");
    ...
    showTimeoutPopup() {
        ...
        ViewManager.instance.show("Game/OverDialog");
